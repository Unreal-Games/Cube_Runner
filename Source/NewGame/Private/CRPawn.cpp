...
{
	ACRObstacles* Obstacle = Cast<ACRObstacles>(Other);
...
{
	if (!bGameEnded) {
		FVector Impulse = FVector(0.0f, Mass * SidePush * DeltaSeconds* AxisValue ,  0.0f);
...
void ACRPawn::Tick(float DeltaTime)
{
	...
	if (!bGameEnded)
	{
		FVector Impulse = FVector(Mass * ForwardPush * DeltaTime, 0.0f, 0.0f);
